...
#>>>>>>> Stashed changes
        try:
            # response = requests.get(self.url, headers=self.headers, data = self.payload, timeout=REQUEST_TIMEOUT) # Dave's original
            response = requests.get(url, headers=self.headers, data = self.payload, timeout=REQUEST_TIMEOUT)    # Tim's modification
            response.raise_for_status()
        except requests.HTTPError as exception:
            return exception
        data = response.json().get('data')
    ...
    def create_apps(self, app_name, app_description):
        ...
        response = requests.post(self.url, headers=self.headers, data = self.payload, timeout=REQUEST_TIMEOUT)
        print(response.text.encode('utf8'))
        #print("API Token: "+ str(self.token))

        location = response.headers.get('Location', None, timeout=REQUEST_TIMEOUT)
        if location:
